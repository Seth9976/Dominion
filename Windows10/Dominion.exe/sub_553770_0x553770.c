// 函数: sub_553770
// 地址: 0x553770
// 来自: E:/Dominion/steamapps/common/Dominion/Dominion.exe.bndb

int32_t __saved_esi_1
int32_t __saved_esi = __saved_esi_1
int32_t eax
sub_561e00(eax, 0, 1, 0)
void* eax_1 = sub_573400()
sub_590760(eax_1, *(eax_1 + 0xc), *(eax_1 + 4), 1, 2, 0, 0)
void* eax_2 = sub_573400()
int32_t ecx_1 = *(eax_2 + 0xc)

if (ecx_1 != 0xffffffff)
    int32_t eax_3 = *(eax_2 + 4)
    int32_t ecx_2 = ecx_1 * 0x5a30
    *(ecx_2 + eax_3 + 0x17558) |= 2
    int32_t esi_1 = 0
    int32_t i = 7
    int32_t* result = *(sub_573400() + 4) + 0x1594
    
    do
        if (*result != 0 && result[2] == 0)
            esi_1 += 1
        
        i += 1
        result = &result[4]
    while (i s< 0x21)
    
    if (esi_1 s>= 1)
        uint32_t eax_6
        int32_t ecx_3
        eax_6, ecx_3 = sub_56e9c0(1)
        int32_t var_8_1 = ecx_3
        result = sub_561e00(eax_6, 0, 1, 0)
    
    if (esi_1 s< 2)
        return result
    
    void* eax_7 = sub_573400()
    sub_590760(eax_7, *(eax_7 + 0xc), *(eax_7 + 4), 2, 1, 0, 0)
    eax_2 = sub_573400()
    int32_t ecx_5 = *(eax_2 + 0xc)
    
    if (ecx_5 != 0xffffffff)
        int32_t eax_8 = *(eax_2 + 4)
        int32_t edx_2 = ecx_5 * 0x5a30
        *(edx_2 + eax_8 + 0x17558) |= 8
        return sub_561af0(eax_8, 0, 1, 0)

sub_63b870(eax_2, &data_801800, "c.activePlayer != PLAYER_NONE", 
    "C:\x\ax2017\Jams\Dominion\code\DomCardUtils.cpp", 0x52, "SetGainFlag")

if (sub_63bc30() != 0)
    breakpoint

sub_63bb00()
noreturn
