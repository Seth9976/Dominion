// 函数: sub_4cb050
// 地址: 0x4cb050
// 来自: E:/Dominion/steamapps/common/Dominion/Dominion.exe.bndb

int32_t var_8 = 0xffffffff
int32_t (* var_c)(struct EHRegistrationNode* arg1) = __ehhandler$___std_smf_hermite@12
TEB* fsbase
struct _EXCEPTION_REGISTRATION_RECORD* ExceptionList = fsbase->NtTib.ExceptionList
char* var_14 = arg1
int32_t __saved_ebp
int32_t var_24 = __security_cookie ^ &__saved_ebp
fsbase->NtTib.ExceptionList = &ExceptionList
char* result_1
char* result_6 = result_1
char* esi = *(result_6 + 4)
sub_63d720(&var_14, "btnBack")
char* eax_3 = var_14
char* const result_2 = &data_801800
char* const ecx_1 = &data_801800

if (eax_3 != 0)
    ecx_1 = eax_3

int32_t ecx_3

while (true)
    char edx_1 = *ecx_1
    char temp0_1 = *esi
    bool c_1 = edx_1 u< temp0_1
    
    if (edx_1 == temp0_1)
        if (edx_1 == 0)
            ecx_3 = 0
            break
        
        edx_1 = ecx_1[1]
        char temp1_1 = esi[1]
        c_1 = edx_1 u< temp1_1
        
        if (edx_1 == temp1_1)
            ecx_1 = &ecx_1[2]
            esi = &esi[2]
            
            if (edx_1 != 0)
                continue
            
            ecx_3 = 0
            break
    
    ecx_3 = sbb.d(ecx_1, ecx_1, c_1) | 1
    break

if (ecx_3 == 0 || *(result_6 + 0x18) == 2)
    result_6.b = 1
else
    result_6.b = 0

int32_t var_8_1 = 0

if (data_cf65bc != 0 && eax_3 != 0 && *eax_3 != 0)
    char* eax_4 = sub_63d4e0(&var_14)
    int32_t temp2_1 = *(eax_4 + 4)
    *(eax_4 + 4) -= 1
    
    if (temp2_1 == 1)
        sub_64c080(eax_4, *(eax_4 + 0xc) + 0x10)
        var_14 = &data_801800

int32_t var_8_2 = 0xffffffff

if (result_6.b != 0)
    if (data_8db664 == 0x18)
        data_8db660 = 0x17
    else
        int32_t eax_5
        
        if (data_8db5c4 != 0x27)
            if (data_8db5d4 == 0x27)
                eax_5 = data_8db5d8
                goto label_4cb146
            
            data_8db660 = 4
        else
            eax_5 = data_8db5c8
        label_4cb146:
            
            if (eax_5 == 0)
                data_8db660 = 4
            else
                sub_4d46e0(5, 0)

char* result_7 = result_1
char* esi_1 = *(result_7 + 4)
sub_63d720(&result_1, "btnTutorial")
char* result_3 = result_1
char* const result_8 = &data_801800

if (result_3 != 0)
    result_8 = result_3

int32_t esi_3

while (true)
    char edx_4 = *result_8
    char temp3_1 = *esi_1
    bool c_2 = edx_4 u< temp3_1
    
    if (edx_4 == temp3_1)
        if (edx_4 == 0)
            esi_3 = 0
            break
        
        edx_4 = result_8[1]
        char temp4_1 = esi_1[1]
        c_2 = edx_4 u< temp4_1
        
        if (edx_4 == temp4_1)
            result_8 = &result_8[2]
            esi_1 = &esi_1[2]
            
            if (edx_4 != 0)
                continue
            
            esi_3 = 0
            break
    
    esi_3 = sbb.d(esi_1, esi_1, c_2) | 1
    break

int32_t var_8_3 = 1

if (data_cf65bc != 0 && result_3 != 0 && *result_3 != 0)
    char* eax_6 = sub_63d4e0(&result_1)
    int32_t temp5_1 = *(eax_6 + 4)
    *(eax_6 + 4) -= 1
    
    if (temp5_1 == 1)
        sub_64c080(eax_6, *(eax_6 + 0xc) + 0x10)
        result_1 = &data_801800

int32_t var_8_4 = 0xffffffff

if (esi_3 == 0)
    data_8db660 = 0x23

char* esi_4 = *(result_7 + 4)
sub_63d720(&result_1, "btnRulesItem")
char* result_4 = result_1
char* const result_9 = &data_801800

if (result_4 != 0)
    result_9 = result_4

int32_t esi_6

while (true)
    char edx_7 = *result_9
    char temp6_1 = *esi_4
    bool c_3 = edx_7 u< temp6_1
    
    if (edx_7 == temp6_1)
        if (edx_7 == 0)
            esi_6 = 0
            break
        
        edx_7 = result_9[1]
        char temp7_1 = esi_4[1]
        c_3 = edx_7 u< temp7_1
        
        if (edx_7 == temp7_1)
            result_9 = &result_9[2]
            esi_4 = &esi_4[2]
            
            if (edx_7 != 0)
                continue
            
            esi_6 = 0
            break
    
    esi_6 = sbb.d(esi_4, esi_4, c_3) | 1
    break

int32_t var_8_5 = 2

if (data_cf65bc != 0 && result_4 != 0 && *result_4 != 0)
    char* eax_7 = sub_63d4e0(&result_1)
    int32_t temp10_1 = *(eax_7 + 4)
    *(eax_7 + 4) -= 1
    
    if (temp10_1 == 1)
        sub_64c080(eax_7, *(eax_7 + 0xc) + 0x10)
        result_1 = &data_801800

int32_t var_8_6 = 0xffffffff

if (esi_6 == 0)
    int32_t eax_9 = *(result_7 + 8) * 3
    int32_t ecx_12 = *((eax_9 << 3) + &data_7e7678)
    
    if (ecx_12 == esi_6)
        ShellExecuteA(nullptr, "open", *((eax_9 << 3) + &data_7e767c), nullptr, nullptr, 
            SW_SHOWNORMAL)
    else
        if (ecx_12 - esi_6 != 1)
            sub_63b870(eax_9, &data_801800, "Halt", 
                "C:\x\ax2017\Jams\Shared\TggGame\code\GameDialogs.cpp", 0x23b0, "RulesClick")
            
            if (sub_63bc30() != 0)
                breakpoint
            
            sub_63bb00()
            noreturn
        
        data_8db660 = *((eax_9 << 3) + &data_7e7680)

char* esi_7 = *(result_7 + 4)
sub_63d720(&result_1, "btn_basic_rules")
char* result_5 = result_1
char* result_10 = &data_801800

if (result_5 != 0)
    result_10 = result_5

int32_t esi_9

while (true)
    char edx_10 = *result_10
    char temp11_1 = *esi_7
    bool c_4 = edx_10 u< temp11_1
    
    if (edx_10 == temp11_1)
        if (edx_10 == 0)
            esi_9 = 0
            break
        
        edx_10 = result_10[1]
        char temp12_1 = esi_7[1]
        c_4 = edx_10 u< temp12_1
        
        if (edx_10 == temp12_1)
            result_10 = &result_10[2]
            esi_7 = &esi_7[2]
            
            if (edx_10 != 0)
                continue
            
            esi_9 = 0
            break
    
    esi_9 = sbb.d(esi_7, esi_7, c_4) | 1
    break

int32_t var_8_7 = 3

if (data_cf65bc != 0 && result_5 != 0 && *result_5 != 0)
    char* eax_11 = sub_63d4e0(&result_1)
    int32_t temp13_1 = *(eax_11 + 4)
    *(eax_11 + 4) -= 1
    
    if (temp13_1 == 1)
        sub_64c080(eax_11, *(eax_11 + 0xc) + 0x10)

int32_t var_8_8 = 0xffffffff

if (esi_9 == 0)
    data_8db660 = 0x7d0

char* esi_10 = *(result_7 + 4)
sub_63d720(&result_1, "btn_expansions")
char* result = result_1

if (result != 0)
    result_2 = result

int32_t esi_12

while (true)
    char ecx_19 = *result_2
    char temp14_1 = *esi_10
    bool c_5 = ecx_19 u< temp14_1
    
    if (ecx_19 == temp14_1)
        if (ecx_19 == 0)
            esi_12 = 0
            break
        
        ecx_19 = result_2[1]
        char temp15_1 = esi_10[1]
        c_5 = ecx_19 u< temp15_1
        
        if (ecx_19 == temp15_1)
            result_2 = &result_2[2]
            esi_10 = &esi_10[2]
            
            if (ecx_19 != 0)
                continue
            
            esi_12 = 0
            break
    
    esi_12 = sbb.d(esi_10, esi_10, c_5) | 1
    break

int32_t var_8_9 = 4

if (data_cf65bc != 0 && result != 0 && *result != 0)
    result = sub_63d4e0(&result_1)
    int32_t temp16_1 = *(result + 4)
    *(result + 4) -= 1
    
    if (temp16_1 == 1)
        sub_64c080(result, *(result + 0xc) + 0x10)

if (esi_12 == 0)
    data_8db660 = 0x18

result.b = 0
fsbase->NtTib.ExceptionList = ExceptionList
return result
