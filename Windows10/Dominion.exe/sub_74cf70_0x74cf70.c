// 函数: sub_74cf70
// 地址: 0x74cf70
// 来自: E:/Dominion/steamapps/common/Dominion/Dominion.exe.bndb

__security_cookie
uint32_t eax_2 = data_19e2778

if (*(eax_2 + 4) != 0)
    RECT var_18
    __builtin_memset(&var_18, 0, 0x10)
    SendMessageA(arg1, 0x198, SendMessageA(arg1, 0x188, 0, 0), &var_18)
    void* edx_1 = data_19e2778
    int32_t Y_2 = var_18.top + 1
    var_18.top = Y_2
    int32_t X_1 = *(edx_1 + 0x24) + 1
    var_18.left = X_1
    void* ecx = *(edx_1 + 4)
    int32_t eax_4 = *(ecx + 0x14)
    HWND hWnd
    int32_t X
    int32_t Y
    int32_t nWidth
    int32_t nHeight
    
    if (eax_4 u> 0x63)
    label_74d0c1:
        
        if (*(edx_1 + 0x1c) != 0)
            int32_t right = var_18.right
            
            if (right - X_1 s> 0x13)
                X_1 = right - 0x13
                right -= 2
                var_18.left = X_1
                var_18.right = right
            
            var_18.top = Y_2 + 2
            int32_t var_40_4 = 1
            var_18.bottom -= 2
            nHeight = var_18.bottom - 2 - (Y_2 + 2)
            nWidth = right - X_1
            Y = Y_2 + 2
            X = X_1
            hWnd = *(edx_1 + 0x1c)
            goto label_74d0ff
        
        eax_2 = SetFocus(*(edx_1 + 0x14))
    else
        eax_2 = zx.d(lookup_table_74d148[eax_4])
        
        switch (eax_2)
            case 0
                HWND hWnd_2 = *(edx_1 + 0x1c)
                
                if (hWnd_2 != 0)
                    int32_t var_40_3 = 1
                    nHeight = var_18.bottom - Y_2
                    nWidth = var_18.right - X_1
                    Y = Y_2
                    X = X_1
                    hWnd = hWnd_2
                label_74d0ff:
                    MoveWindow(hWnd, X, Y, nWidth, nHeight, 1)
                    ShowWindow(*(data_19e2778 + 0x1c), SW_SHOW)
                    eax_2 = SetFocus(*(data_19e2778 + 0x1c))
                else
                    eax_2 = SetFocus(*(edx_1 + 0x14))
            case 2
                *(ecx + 8) = &data_8901e0
                BOOL eax_5 =
                    RedrawWindow(arg1, &var_18, nullptr, RDW_INVALIDATE | RDW_ERASE | RDW_UPDATENOW)
                CookieCheckFunction(eax_5)
                return eax_5
            case 3
                HWND hWnd_1 = *(edx_1 + 0x1c)
                
                if (hWnd_1 == 0 || *(edx_1 + 0x20) == 0)
                    eax_2 = SetFocus(*(edx_1 + 0x14))
                else
                    uint128_t xmm1
                    xmm1.d = var_18.left
                    xmm1:4.d = var_18.top
                    xmm1:8.d = var_18.right
                    xmm1:0xc.d = var_18.bottom
                    int32_t eax_12
                    int32_t edx_2
                    edx_2:eax_12 = sx.q(_mm_bsrli_si128(xmm1, 8) - X_1)
                    int32_t nWidth_1 = (eax_12 - edx_2) s>> 1
                    int32_t X_2 = nWidth_1 + X_1
                    MoveWindow(hWnd_1, X_1, Y_2, nWidth_1, _mm_bsrli_si128(xmm1, 0xc) - Y_2, 1)
                    int32_t Y_1 = xmm1:4.d
                    MoveWindow(*(data_19e2778 + 0x20), X_2, Y_1, xmm1:8.d - X_2, xmm1:0xc.d - Y_1, 
                        1)
                    ShowWindow(*(data_19e2778 + 0x20), SW_SHOW)
                    ShowWindow(*(data_19e2778 + 0x1c), SW_SHOW)
                    eax_2 = SetFocus(*(data_19e2778 + 0x1c))
            case 4
                goto label_74d0c1

CookieCheckFunction(eax_2)
return eax_2
