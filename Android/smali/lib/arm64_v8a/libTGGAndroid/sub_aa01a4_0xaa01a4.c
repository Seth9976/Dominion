// 函数: sub_aa01a4
// 地址: 0xaa01a4
// 来自: E:\torrent\Cursor\Dominion_1.0.3315\split_config.arm64_v8a\lib\arm64-v8a\libTGGAndroid.so

struct std::__ndk1::__function::__base<bool (CardID)>::std::__ndk1::__function::__func<Necromancer()::$_9, std::__ndk1::allocator<Necromancer()::$_9>, bool (CardID)>::VTable
    ** result = operator new(0x10)
*result = &_vtable_for_std::__ndk1::__function::__func<Necromancer()::$_9, std::__ndk1::allocator<Necromancer()::$_9>, bool (CardID)>{for `std::__ndk1::__function::__base<bool (CardID)>'}
return result
