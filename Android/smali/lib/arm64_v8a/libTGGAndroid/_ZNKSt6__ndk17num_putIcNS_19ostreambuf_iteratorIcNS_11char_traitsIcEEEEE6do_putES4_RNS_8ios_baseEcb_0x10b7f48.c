// 函数: _ZNKSt6__ndk17num_putIcNS_19ostreambuf_iteratorIcNS_11char_traitsIcEEEEE6do_putES4_RNS_8ios_baseEcb
// 地址: 0x10b7f48
...
if ((zx.d(*(arg3 + 8)) & 1) != 0)
    ...
    int64_t (* const var_70)() = std::__ndk1::numpunct<char>::id
    ...
    if (*std::__ndk1::numpunct<char>::id != -1)
        ...
        var_50 = &var_70
        std::__ndk1::__call_once(std::__ndk1::numpunct<char>::id, &var_58, sub_10dd4b0)
    
    void* var_78
    int64_t x8_5 = *(var_78 + 0x10)
    int64_t x9_2 = sx.q(*(std::__ndk1::numpunct<char>::id + 8)) - 1
    int64_t* x21_2
    ...
    while (true)
        if ((x8_7 & 1) != 0)
            if (x21_3 == var_60_1 + var_68_1)
                ...
                break
        else if (x21_3 == &var_70 + (zx.q(x8_7 u>> 1) & 0x7f) + 1)
            break
        
        if (result != 0)
            char* x8_13 = *(result + 0x30)
            char x1_2 = *x21_3
            
            if (x8_13 != *(result + 0x38))
                *(result + 0x30) = &x8_13[1]
                *x8_13 = x1_2
            else if ((*(*result + 0x68))(result, x1_2) == 0xffffffff)
                result = nullptr
