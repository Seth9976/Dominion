// 函数: sub_cb373c
// 地址: 0xcb373c
// 来自: E:\torrent\Cursor\Dominion_1.0.3315\split_config.arm64_v8a\lib\arm64-v8a\libTGGAndroid.so

if ((zx.d(arg29) & 1) != 0)
    operator delete(arg31)

void* __offset(
    vtable_for_std::__ndk1::basic_ostringstream<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >, 
    0x40) arg_1e0 = _vtable_for_std::__ndk1::basic_ostringstream<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >
    + 0x40
void* __offset(
    vtable_for_std::__ndk1::basic_ostringstream<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >, 
    0x18) arg_170 = _vtable_for_std::__ndk1::basic_ostringstream<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >
    + 0x18
void* __offset(
    vtable_for_std::__ndk1::basic_stringbuf<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >, 
    0x10) arg_178 = _vtable_for_std::__ndk1::basic_stringbuf<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >
    + 0x10

if ((zx.d(arg47) & 1) != 0)
    operator delete(arg49)

std::__ndk1::basic_streambuf<char, std::__ndk1::char_traits<char> >::~basic_streambuf()
std::__ndk1::basic_ostream<char, std::__ndk1::char_traits<char> >::~basic_ostream()
std::__ndk1::basic_ios<char, std::__ndk1::char_traits<char> >::~basic_ios()

if (arg35 != 0)
    (*(*arg35 + 0x38))()

if (arg33 != 0)
    Botan::deallocate_memory(arg33, arg34 - arg33, 1)

void* arg_120
void* x0_6 = arg_120

if (x0_6 != 0)
    arg36 = x0_6
    operator delete(x0_6)

__cxa_begin_catch(arg1)
__cxa_end_catch()
int64_t x20_1 = arg2
int64_t x27 = arg3
label_cb28cc:
char* arg_138
(*(*arg4 + 0x18))(arg4, zx.q(arg5), zx.q(arg7), &arg_138)
char arg_e0
(*(*arg4 + 0x10))(arg4, zx.q(arg5), zx.q(arg7), x27, arg8, &arg_e0)
uint64_t x8_31 = zx.q(arg_170.b)
int32_t temp0_1 = x8_31.d & 1
void* __offset(
    vtable_for_std::__ndk1::basic_stringbuf<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >, 
    0x10) x2_8

if (temp0_1 == 0)
    x2_8 = x8_31 u>> 1
else
    x2_8 = arg_178

void* __offset(
    vtable_for_std::__ndk1::basic_stringbuf<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >, 
    0x10) x1_13

x1_13 = temp0_1 == 0 ? arg21 : arg42

std::__ndk1::__put_character_sequence<char, std::__ndk1::char_traits<char> >(arg27, x1_13, x2_8)
label_cb2920:

if ((zx.d(arg_170.b) & 1) != 0)
label_cb2928:
    operator delete(arg42)

while (true)
    if ((zx.d(arg_e0) & 1) != 0)
        operator delete(arg33)
    
    while (true)
        Botan::BER_Decoder::get_next_object()
        void* __offset(
            vtable_for_std::__ndk1::basic_ostringstream<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >, 
            0x18) x8_43 = arg_170
        
        if (arg39 != 0)
            arg40 = arg39
            Botan::deallocate_memory(arg39, arg41 - arg39, 1)
        
        int64_t* x0_36 = arg52
        arg52 = nullptr
        arg39.o = arg_178.o
        arg41 = arg43
        
        if (x0_36 != 0)
            (*(*x0_36 + 0x38))()
        
        if (arg50 != 0)
            void* arg_298 = arg50
            Botan::deallocate_memory(arg50, arg51 - arg50, 1)
        
        char* x0_38 = arg_138
        
        if (x0_38 != 0)
            arg38 = x0_38
            operator delete(x0_38)
        
        arg5 = x8_43.d
        
        if (arg5 == 0xff00)
            void* result = arg39
            
            if (result != 0)
                arg40 = result
                result = Botan::deallocate_memory(result, arg41 - result, 1)
            
            if (*(arg16 + 0x28) == arg53)
                return result
            
            __stack_chk_fail()
            noreturn
        
        int64_t x21_1 = x27
        int64_t x19_1 = x20_1
        arg7 = x8_43:4.d
        int64_t* arg_190 = nullptr
        __builtin_memset(arg25, 0, 0x30)
        struct std::__ndk1::__function::__base<void (uint8_t const*, uint64_t)>::std::__ndk1::__function::__func<Botan::DER_Encoder::DER_Encoder(std::__ndk1::vector<uint8_t, std::__ndk1::allocator<uint8_t> >&)::$_2, std::__ndk1::allocator<Botan::DER_Encoder::DER_Encoder(std::__ndk1::vector<uint8_t, std::__ndk1::allocator<uint8_t> >&)::$_2>, void (uint8_t const*, uint64_t)>::VTable
            * const arg_280 = &_vtable_for_std::__ndk1::__function::__func<Botan::DER_Encoder::DER_Encoder(std::__ndk1::vector<uint8_t, std::__ndk1::allocator<uint8_t> >&)::$_2, std::__ndk1::allocator<Botan::DER_Encoder::DER_Encoder(std::__ndk1::vector<uint8_t, std::__ndk1::allocator<uint8_t> >&)::$_2>, void (uint8_t const*, uint64_t)>{for `std::__ndk1::__function::__base<void (uint8_t const*, uint64_t)>'}
        __builtin_memset(&arg_138, 0, 0x18)
        char** arg_288 = &arg_138
        arg51 = &arg_280
        sub_f2410c(&arg_280, &arg_170)
        
        if (&arg_280 == arg51)
            (*arg51)->vFunc_4()
        else if (arg51 != 0)
            (*arg51)->j_operator delete()
        
        Botan::DER_Encoder::add_object(&arg_170, zx.q(arg5), zx.q(arg7), arg39)
        void* x28_1 = arg47.q
        
        if (x28_1 != 0)
            void* x23_2 = arg48
            void* x0_12
            
            if (x23_2 == x28_1)
                x0_12 = x28_1
            else
                do
                    x23_2 -= 0x38
                    sub_f230d4(arg6, x23_2)
                while (x28_1 != x23_2)
                
                x0_12 = arg47.q
            
            arg48 = x28_1
            operator delete(x0_12)
        
        void* arg_1a8
        
        if (arg45 != 0)
            arg_1a8 = arg45
            Botan::deallocate_memory(arg45, arg46 - arg45, 1)
        
        if (&arg_170 == arg_190)
            (*(*arg_190 + 0x20))()
        else if (arg_190 != 0)
            (*(*arg_190 + 0x28))()
        
        Botan::BER_Decoder::BER_Decoder(&arg_280)
        arg8 = arg40 - arg39
        
        if ((arg7 & 0x20) != 0)
            Botan::BER_Decoder::BER_Decoder(&arg_170, arg39)
            x20_1 = x19_1
            x27 = x21_1
            
            if (arg24 u>= x21_1)
                arg_120.w = 0
                (*(*arg4 + 0x10))(arg4, zx.q(arg5), zx.q(arg7), x27, arg8, &arg_120)
                uint64_t x8_36 = zx.q(arg_e0)
                int32_t temp1_1 = x8_36.d & 1
                uint64_t x2_10
                
                if (temp1_1 == 0)
                    x2_10 = x8_36 u>> 1
                else
                    x2_10 = arg32
                
                char* x1_15
                
                x1_15 = temp1_1 == 0 ? arg23 : arg33
                
                std::__ndk1::__put_character_sequence<char, std::__ndk1::char_traits<char> >(arg27, 
                    x1_15, x2_10)
                
                if ((zx.d(arg_e0) & 1) != 0)
                    operator delete(arg33)
                
                if ((zx.d(arg_120.b) & 1) != 0)
                    operator delete(arg37)
                
                Botan::ASN1_Formatter::decode(arg4, arg27, &arg_170)
            else
                (*(*arg4 + 0x18))(arg4, zx.q(arg5), zx.q(arg7), &arg_138)
                (*(*arg4 + 0x10))(arg4, zx.q(arg5), zx.q(arg7), x27, arg8, &arg_120)
                uint64_t x8_24 = zx.q(arg_e0)
                int32_t temp2_1 = x8_24.d & 1
                uint64_t x2_5
                
                if (temp2_1 == 0)
                    x2_5 = x8_24 u>> 1
                else
                    x2_5 = arg32
                
                char* x1_10
                
                x1_10 = temp2_1 == 0 ? arg23 : arg33
                
                std::__ndk1::__put_character_sequence<char, std::__ndk1::char_traits<char> >(arg27, 
                    x1_10, x2_5)
                
                if ((zx.d(arg_e0) & 1) != 0)
                    operator delete(arg33)
                
                if ((zx.d(arg_120.b) & 1) != 0)
                    operator delete(arg37)
            
            if (arg45 != 0)
                (*(*arg45 + 0x38))()
            
            if (arg42 != 0)
                Botan::deallocate_memory(arg42, arg_190 - arg42, 1)
        else
            x20_1 = x19_1
            x27 = x21_1
            int32_t arg_1d8
            int64_t arg_268
            int32_t arg_270
            
            if ((arg7 & 0xc0) == 0)
                int16_t arg_a8
                void* __offset(
                    vtable_for_std::__ndk1::basic_stringbuf<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >, 
                    0x10) x0_73
                
                if (arg5 == 6)
                    arg_170 = arg15
                    __builtin_memset(x20_1, 0, 0x18)
                    Botan::OID::decode_from(&arg_170)
                    Botan::OIDS::oid2str_or_empty(&arg_170)
                    uint64_t x8_50 = zx.q(arg_e0)
                    uint64_t x8_51
                    
                    if ((x8_50.d & 1) == 0)
                        x8_51 = x8_50 u>> 1
                    else
                        x8_51 = arg32
                    
                    if (x8_51 == 0)
                        Botan::OID::to_string()
                        
                        if ((zx.d(arg_e0) & 1) != 0)
                            operator delete(arg33)
                        
                        arg_e0.o = arg_120.o
                        arg33 = arg37
                    else
                        Botan::OID::to_string()
                        int128_t* x0_42
                        int128_t v0_2
                        x0_42, v0_2 = std::__ndk1::basic_string<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >::insert(
                            &arg_a8, nullptr)
                        arg31 = x0_42[1].q
                        arg29.o = *x0_42
                        __builtin_memset(x0_42, 0, 0x18)
                        int128_t* x0_44
                        int128_t v0_3
                        x0_44, v0_3 = std::__ndk1::basic_string<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >::append(
                            &arg29)
                        arg37 = x0_44[1].q
                        arg_120.o = *x0_44
                        __builtin_memset(x0_44, 0, 0x18)
                        char* x1_21
                        
                        if ((zx.d(arg_120.b) & 1) == 0)
                            x1_21 = arg20
                        else
                            x1_21 = arg37
                        
                        std::__ndk1::basic_string<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >::append(
                            &arg_e0, x1_21)
                        
                        if ((zx.d(arg_120.b) & 1) == 0)
                            if ((zx.d(arg29) & 1) != 0)
                                goto label_cb2e40
                            
                            goto label_cb2b5c
                        
                        operator delete(arg37)
                        
                        if ((zx.d(arg29) & 1) == 0)
                        label_cb2b5c:
                            
                            if ((zx.d(arg_a8.b) & 1) != 0)
                                operator delete(arg28)
                        else
                        label_cb2e40:
                            operator delete(arg31)
                            
                            if ((zx.d(arg_a8.b) & 1) != 0)
                                operator delete(arg28)
                    
                    (*(*arg4 + 0x10))(arg4, 6, zx.q(arg7), x27, arg8, &arg_e0)
                    uint64_t x8_83 = zx.q(arg_120.b)
                    int32_t temp9_1 = x8_83.d & 1
                    void* x2_21
                    
                    if (temp9_1 == 0)
                        x2_21 = x8_83 u>> 1
                    else
                        x2_21 = arg36
                    
                    char* x1_32
                    
                    x1_32 = temp9_1 == 0 ? arg20 : arg37
                    
                    std::__ndk1::__put_character_sequence<char, std::__ndk1::char_traits<char> >(
                        arg27, x1_32, x2_21)
                    
                    if ((zx.d(arg_120.b) & 1) == 0)
                        if ((zx.d(arg_e0) & 1) != 0)
                            goto label_cb2ed8
                        
                        goto label_cb2eb0
                    
                    operator delete(arg37)
                    
                    if ((zx.d(arg_e0) & 1) == 0)
                    label_cb2eb0:
                        x0_73 = arg_178
                        arg_170 = _vtable_for_Botan::OID + 0x10
                        
                        if (x0_73 == 0)
                            continue
                    else
                    label_cb2ed8:
                        operator delete(arg33)
                        x0_73 = arg_178
                        arg_170 = _vtable_for_Botan::OID + 0x10
                        
                        if (x0_73 == 0)
                            continue
                    
                label_cb2eec:
                    arg42 = x0_73
                    operator delete(x0_73)
                    continue
                else if ((arg5 & 0xfffffff7) != 2)
                    if (arg5 == 5)
                        arg_e0.w = 0
                        (*(*arg4 + 0x10))(arg4, 5, zx.q(arg7), x27, arg8, &arg_e0)
                        uint64_t x8_93 = zx.q(arg_170.b)
                        int32_t temp3_1 = x8_93.d & 1
                        uint64_t x2_23
                        
                        if (temp3_1 == 0)
                            x2_23 = x8_93 u>> 1
                        else
                            x2_23 = arg_178
                        
                        void* x1_34
                        
                        x1_34 = temp3_1 == 0 ? arg21 : arg42
                        
                        std::__ndk1::__put_character_sequence<char, std::__ndk1::char_traits<char> >(
                            arg27, x1_34, x2_23)
                        goto label_cb2920
                    
                    if (arg5 == 1)
                        Botan::BER_Decoder::decode(&arg_280, &arg_120, 1)
                        uint32_t x8_71 = zx.d(arg_120.b)
                        char const* const x1_30
                        
                        if (x8_71 == 0)
                            x1_30 = "false"
                        else
                            x1_30 = "true"
                        
                        size_t x20_4
                        
                        x20_4 = x8_71 == 0 ? 5 : 4
                        
                        arg_e0 = (x20_4.d << 1).b
                        memcpy(arg23, x1_30, x20_4)
                        *((&arg_e0 | x20_4) + 1) = 0
                        (*(*arg4 + 0x10))(arg4, 1, zx.q(arg7), x27, arg8, &arg_e0)
                        uint64_t x8_75 = zx.q(arg_170.b)
                        int32_t temp12_1 = x8_75.d & 1
                        uint64_t x2_19
                        
                        if (temp12_1 == 0)
                            x2_19 = x8_75 u>> 1
                        else
                            x2_19 = arg_178
                        
                        void* x1_31
                        
                        x1_31 = temp12_1 == 0 ? arg21 : arg42
                        
                        std::__ndk1::__put_character_sequence<char, std::__ndk1::char_traits<char> >(
                            arg27, x1_31, x2_19)
                        x20_1 = x19_1
                        
                        if ((zx.d(arg_170.b) & 1) != 0)
                            goto label_cb2928
                        
                        break
                    
                    if (arg5 - 3 u> 1)
                        if (arg5 u<= 0x1e && (1 << arg5 & 0x545c1000) != 0)
                            arg_e0.w = 0
                            arg_170 = arg13
                            __builtin_memset(x20_1, 0, 0x18)
                            std::__ndk1::basic_string<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >::basic_string(
                                arg14)
                            uint64_t x8_106 = zx.q(arg_190.b)
                            
                            if ((x8_106.d & 1) != 0)
                                int64_t x8_137 = arg44
                                void* x9_43 = arg45
                                uint64_t x10_21
                                
                                do
                                    if (x8_137 == 0)
                                        goto label_cb3458
                                    
                                    x10_21 = zx.q(*x9_43)
                                    x9_43 += 1
                                    x8_137 -= 1
                                while (zx.d(*(&data_84a2f8 + x10_21)) != 0)
                            else
                                char* x9_31 = arg10
                                uint64_t x8_107 = x8_106 u>> 1
                                uint64_t x10_14
                                
                                do
                                    if (x8_107 == 0)
                                    label_cb3458:
                                        arg_1a8.d = 0x13
                                        
                                        if ((zx.d(arg_e0) & 1) == 0)
                                            goto label_cb3478
                                        
                                        goto label_cb3464
                                    
                                    x10_14 = zx.q(*x9_31)
                                    x9_31 = &x9_31[1]
                                    x8_107 -= 1
                                while (zx.d(*(&data_84a2f8 + x10_14)) != 0)
                            
                            arg_1a8.d = 0xc
                            
                            if ((zx.d(arg_e0) & 1) != 0)
                            label_cb3464:
                                operator delete(arg33)
                            
                        label_cb3478:
                            (*(arg_170 + 8))(&arg_170, &arg_280)
                            (*(*arg4 + 0x10))(arg4, zx.q(arg5), zx.q(arg7), x27, arg8, arg14)
                            uint64_t x8_141 = zx.q(arg_e0)
                            int32_t temp13_1 = x8_141.d & 1
                            uint64_t x2_38
                            
                            if (temp13_1 == 0)
                                x2_38 = x8_141 u>> 1
                            else
                                x2_38 = arg32
                            
                            void* x1_55
                            
                            x1_55 = temp13_1 == 0 ? arg23 : arg33
                            
                            std::__ndk1::__put_character_sequence<char, std::__ndk1::char_traits<char> >(
                                arg27, x1_55, x2_38)
                            
                            if ((zx.d(arg_e0) & 1) != 0)
                                operator delete(arg33)
                            
                            arg_170 = _vtable_for_Botan::ASN1_String + 0x10
                            
                            if ((zx.d(arg_190.b) & 1) != 0)
                                operator delete(arg45)
                            
                            x0_73 = arg_178
                            
                            if (x0_73 == 0)
                                continue
                        else if (arg5 - 0x17 u> 1)
                            std::__ndk1::__put_character_sequence<char, std::__ndk1::char_traits<char> >(
                                std::__ndk1::basic_ostream<char, std::__ndk1::char_traits<char> >::operator<<(std::__ndk1::__put_character_sequence<char, std::__ndk1::char_traits<char> >(
                                    std::__ndk1::basic_ostream<char, std::__ndk1::char_traits<char> >::operator<<(std::__ndk1::__put_character_sequence<char, std::__ndk1::char_traits<char> >(
                                        arg27, "Unknown ASN.1 tag class=", 0x18)), 
                                    " type=", 6)), 
                                "\n", 1)
                            continue
                        else
                            arg_170 = arg9
                            __builtin_memset(x20_1, 0, 0x18)
                            arg_190.d = 0xff00
                            Botan::ASN1_Time::decode_from(&arg_170)
                            Botan::ASN1_Time::readable_string()
                            (*(*arg4 + 0x10))(arg4, zx.q(arg5), zx.q(arg7), x27, arg8, &arg_120)
                            uint64_t x8_148 = zx.q(arg_e0)
                            int32_t temp10_1 = x8_148.d & 1
                            uint64_t x2_40
                            
                            if (temp10_1 == 0)
                                x2_40 = x8_148 u>> 1
                            else
                                x2_40 = arg32
                            
                            char* x1_57
                            
                            x1_57 = temp10_1 == 0 ? arg23 : arg33
                            
                            std::__ndk1::__put_character_sequence<char, std::__ndk1::char_traits<char> >(
                                arg27, x1_57, x2_40)
                            
                            if ((zx.d(arg_e0) & 1) != 0)
                                operator delete(arg33)
                            
                            if ((zx.d(arg_120.b) & 1) == 0)
                                continue
                            else
                                operator delete(arg37)
                                continue
                        
                        goto label_cb2eec
                else
                    arg43 = -1
                    __builtin_memset(&arg_170, 0, 0x18)
                    arg_190.d = 1
                    
                    if (arg5 == 0xa)
                        Botan::BER_Decoder::decode(&arg_280, &arg_170, 0xa)
                    else if (arg5 == 2)
                        Botan::BER_Decoder::decode(&arg_280, &arg_170, 2)
                    
                    Botan::BigInt::encode(&arg_170)
                    char* x23_3 = arg_e0.q
                    uint64_t x8_90 = arg32
                    
                    if (x23_3 == x8_90)
                        if (arg33 == x23_3)
                            char* x0_90 = operator new(1)
                            x8_90 = &x0_90[1]
                            *x0_90 = 0
                            arg_e0.q = x0_90
                            arg32 = x8_90
                            arg33 = x8_90
                            
                            if (x23_3 != 0)
                                operator delete(x23_3)
                                x8_90 = arg32
                        else
                            *x23_3 = 0
                            x8_90 = &x23_3[1]
                            arg32 = &x23_3[1]
                    
                    uint8_t* x0_92 = arg_e0.q
                    Botan::hex_encode(x0_92, x8_90 - x0_92, true)
                    (*(*arg4 + 0x10))(arg4, zx.q(arg5), zx.q(arg7), x27, arg8, &arg29)
                    uint64_t x8_114 = zx.q(arg_120.b)
                    int32_t temp6_1 = x8_114.d & 1
                    uint64_t x2_32
                    
                    if (temp6_1 == 0)
                        x2_32 = x8_114 u>> 1
                    else
                        x2_32 = arg36
                    
                    char* x1_44
                    
                    x1_44 = temp6_1 == 0 ? arg20 : arg37
                    
                    std::__ndk1::__put_character_sequence<char, std::__ndk1::char_traits<char> >(
                        arg27, x1_44, x2_32)
                    
                    if ((zx.d(arg_120.b) & 1) == 0)
                        if ((zx.d(arg29) & 1) != 0)
                            goto label_cb3258
                        
                        goto label_cb3210
                    
                    operator delete(arg37)
                    uint64_t x0_95
                    
                    if ((zx.d(arg29) & 1) == 0)
                    label_cb3210:
                        x0_95 = arg_e0.q
                        
                        if (x0_95 != 0)
                            arg32 = x0_95
                            operator delete(x0_95)
                    else
                    label_cb3258:
                        operator delete(arg31)
                        x0_95 = arg_e0.q
                        
                        if (x0_95 != 0)
                            arg32 = x0_95
                            operator delete(x0_95)
                    void* __offset(
                        vtable_for_std::__ndk1::basic_ostringstream<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >, 
                        0x18) x0_96 = arg_170
                    
                    if (x0_96 == 0)
                        continue
                    else
                        arg_178 = x0_96
                        Botan::deallocate_memory(x0_96, (arg42 - x0_96) s>> 2, 4)
                        continue
                __builtin_memset(&arg_120, 0, 0x18)
                Botan::BER_Decoder::decode(&arg_280, &arg_120, zx.q(arg5), zx.q(arg5))
                
                if (arg24 u>= x27)
                    Botan::BER_Decoder::BER_Decoder(&arg_e0)
                    arg_170 = construction_vtable_for_std::__ndk1::basic_ostream<char, std::__ndk1::char_traits<char> >-in-std::__ndk1::basic_ostringstream<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >
                        + 0x18
                    arg_1e0 = construction_vtable_for_std::__ndk1::basic_ostream<char, std::__ndk1::char_traits<char> >-in-std::__ndk1::basic_ostringstream<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >
                        + 0x40
                    std::__ndk1::ios_base::init(arg19)
                    arg_268 = 0
                    arg_270 = 0xffffffff
                    arg_170 = _vtable_for_std::__ndk1::basic_ostringstream<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >
                        + 0x18
                    arg_1e0 = _vtable_for_std::__ndk1::basic_ostringstream<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >
                        + 0x40
                    std::__ndk1::basic_streambuf<char, std::__ndk1::char_traits<char> >::basic_streambuf()
                    arg_178 = arg12
                    __builtin_memset(arg17, 0, 0x20)
                    arg_1d8 = 0x10
                    Botan::ASN1_Formatter::decode(arg4, &arg_170, &arg_e0)
                    arg_a8 = 0
                    (*(*arg4 + 0x10))(arg4, zx.q(arg5), zx.q(arg7), x27, arg8, &arg_a8)
                    uint64_t x8_126 = zx.q(arg29)
                    int32_t temp7_1 = x8_126.d & 1
                    uint64_t x2_35
                    
                    if (temp7_1 == 0)
                        x2_35 = x8_126 u>> 1
                    else
                        x2_35 = arg30
                    
                    char* x1_49
                    
                    x1_49 = temp7_1 == 0 ? arg18 : arg31
                    
                    std::__ndk1::__put_character_sequence<char, std::__ndk1::char_traits<char> >(
                        arg27, x1_49, x2_35)
                    
                    if ((zx.d(arg29) & 1) != 0)
                        operator delete(arg31)
                    
                    if ((zx.d(arg_a8.b) & 1) != 0)
                        operator delete(arg28)
                    
                    std::__ndk1::basic_stringbuf<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >::str()
                    uint64_t x8_130 = zx.q(arg29)
                    int32_t temp15_1 = x8_130.d & 1
                    uint64_t x2_36
                    
                    if (temp15_1 == 0)
                        x2_36 = x8_130 u>> 1
                    else
                        x2_36 = arg30
                    
                    void* x1_50
                    
                    x1_50 = temp15_1 == 0 ? arg18 : arg31
                    
                    std::__ndk1::__put_character_sequence<char, std::__ndk1::char_traits<char> >(
                        arg27, x1_50, x2_36)
                    
                    if ((zx.d(arg29) & 1) != 0)
                        operator delete(arg31)
                    
                    arg_1e0 = _vtable_for_std::__ndk1::basic_ostringstream<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >
                        + 0x40
                    arg_170 = _vtable_for_std::__ndk1::basic_ostringstream<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >
                        + 0x18
                    arg_178 = _vtable_for_std::__ndk1::basic_stringbuf<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >
                        + 0x10
                    
                    if ((zx.d(arg47) & 1) != 0)
                        operator delete(arg49)
                    
                    std::__ndk1::basic_streambuf<char, std::__ndk1::char_traits<char> >::~basic_streambuf()
                    std::__ndk1::basic_ostream<char, std::__ndk1::char_traits<char> >::~basic_ostream()
                    std::__ndk1::basic_ios<char, std::__ndk1::char_traits<char> >::~basic_ios()
                    
                    if (arg35 != 0)
                        (*(*arg35 + 0x38))()
                    
                    if (arg33 != 0)
                        Botan::deallocate_memory(arg33, arg34 - arg33, 1)
                else
                    (*(*arg4 + 0x18))(arg4, zx.q(arg5), zx.q(arg7), &arg_120)
                    (*(*arg4 + 0x10))(arg4, zx.q(arg5), zx.q(arg7), x27, arg8, &arg_e0)
                    uint64_t x8_99 = zx.q(arg_170.b)
                    int32_t temp8_1 = x8_99.d & 1
                    uint64_t x2_27
                    
                    if (temp8_1 == 0)
                        x2_27 = x8_99 u>> 1
                    else
                        x2_27 = arg_178
                    
                    void* x1_38
                    
                    x1_38 = temp8_1 == 0 ? arg21 : arg42
                    
                    std::__ndk1::__put_character_sequence<char, std::__ndk1::char_traits<char> >(
                        arg27, x1_38, x2_27)
                    
                    if ((zx.d(arg_170.b) & 1) != 0)
                        operator delete(arg42)
                    
                    if ((zx.d(arg_e0) & 1) != 0)
                        operator delete(arg33)
            else
                if (zx.d(arg4[1].b) == 0)
                    goto label_cb28cc
                
                char* x10_1 = arg_138
                void* x8_19 = arg38 - x10_1
                
                if (x8_19 u>= 3 && (zx.d(*x10_1) | 4) == 0x86)
                    size_t i_2 = zx.q(x10_1[1])
                    
                    if (x8_19 - 2 == i_2)
                        void* x10_2 = &x10_1[2]
                        size_t i_1 = i_2
                        size_t i
                        
                        do
                            int32_t x12_1 = sx.d(*x10_2)
                            
                            if ((x12_1 & 0x80000000) != 0)
                                goto label_cb28a4
                            
                            uint64_t x12_2 = zx.q(x12_1.b)
                            
                            if ((zx.d(*(x12_2 + *_ctype_ + 1)) & 7) == 0 && x12_2.d - 0x2d u>= 3
                                    && x12_2.d != 0x3a)
                                goto label_cb28a4
                            
                            i = i_1
                            i_1 -= 1
                            x10_2 += 1
                        while (i != 1)
                        char* fp_2
                        
                        if (i_2.d u>= 0x17)
                            void* x20_5 = (x8_19 + 0xe) & 0xfffffffffffffff0
                            char* x0_86 = operator new(x20_5)
                            fp_2 = x0_86
                            arg32 = i_2
                            arg33 = x0_86
                            arg_e0.q = x20_5 | 1
                            x20_1 = x19_1
                            memcpy(fp_2, &x10_1[2], i_2)
                        else
                            fp_2 = arg23
                            arg_e0 = (i_2.d << 1).b
                            
                            if (i_2.d != 0)
                                memcpy(fp_2, &x10_1[2], i_2)
                        
                        fp_2[i_2] = 0
                        x27 = x21_1
                        (*(*arg4 + 0x10))(arg4, zx.q(arg5), zx.q(arg7), x21_1, x21_1, &arg_e0)
                        uint64_t x8_111 = zx.q(arg_170.b)
                        int32_t temp14_1 = x8_111.d & 1
                        uint64_t x2_30
                        
                        if (temp14_1 == 0)
                            x2_30 = x8_111 u>> 1
                        else
                            x2_30 = arg_178
                        
                        void* __offset(
                            vtable_for_std::__ndk1::basic_stringbuf<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >, 
                            0x10) x1_41
                        
                        x1_41 = temp14_1 == 0 ? arg21 : arg42
                        
                        std::__ndk1::__put_character_sequence<char, std::__ndk1::char_traits<char> >(
                            arg27, x1_41, x2_30)
                        goto label_cb2920
                
            label_cb28a4:
                x27 = x21_1
                
                if (arg24 u< x21_1)
                    goto label_cb28cc
                
                __builtin_memset(&arg_120, 0, 0x18)
                Botan::BER_Decoder::decode(&arg_280, &arg_120, zx.q(arg5), zx.q(arg5))
                Botan::BER_Decoder::BER_Decoder(&arg_e0)
                arg_170 = construction_vtable_for_std::__ndk1::basic_ostream<char, std::__ndk1::char_traits<char> >-in-std::__ndk1::basic_ostringstream<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >
                    + 0x18
                arg_1e0 = construction_vtable_for_std::__ndk1::basic_ostream<char, std::__ndk1::char_traits<char> >-in-std::__ndk1::basic_ostringstream<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >
                    + 0x40
                std::__ndk1::ios_base::init(arg19)
                arg_268 = 0
                arg_270 = 0xffffffff
                arg_170 = _vtable_for_std::__ndk1::basic_ostringstream<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >
                    + 0x18
                arg_1e0 = _vtable_for_std::__ndk1::basic_ostringstream<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >
                    + 0x40
                std::__ndk1::basic_streambuf<char, std::__ndk1::char_traits<char> >::basic_streambuf()
                arg_178 = _vtable_for_std::__ndk1::basic_stringbuf<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >
                    + 0x10
                __builtin_memset(arg17, 0, 0x20)
                arg_1d8 = 0x10
                Botan::ASN1_Formatter::decode(arg4, &arg_170, &arg_e0)
                std::__ndk1::basic_stringbuf<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >::str()
                uint64_t x8_64 = zx.q(arg29)
                int32_t temp4_1 = x8_64.d & 1
                uint64_t x2_16
                
                if (temp4_1 == 0)
                    x2_16 = x8_64 u>> 1
                else
                    x2_16 = arg30
                
                char* x1_26
                
                x1_26 = temp4_1 == 0 ? arg18 : arg31
                
                std::__ndk1::__put_character_sequence<char, std::__ndk1::char_traits<char> >(arg27, 
                    x1_26, x2_16)
                
                if ((zx.d(arg29) & 1) != 0)
                    operator delete(arg31)
                
                arg_1e0 = _vtable_for_std::__ndk1::basic_ostringstream<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >
                    + 0x40
                arg_170 = _vtable_for_std::__ndk1::basic_ostringstream<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >
                    + 0x18
                arg_178 = _vtable_for_std::__ndk1::basic_stringbuf<char, std::__ndk1::char_traits<char>, std::__ndk1::allocator<char> >
                    + 0x10
                
                if ((zx.d(arg47) & 1) != 0)
                    operator delete(arg49)
                
                std::__ndk1::basic_streambuf<char, std::__ndk1::char_traits<char> >::~basic_streambuf()
                std::__ndk1::basic_ostream<char, std::__ndk1::char_traits<char> >::~basic_ostream()
                std::__ndk1::basic_ios<char, std::__ndk1::char_traits<char> >::~basic_ios()
                
                if (arg35 != 0)
                    (*(*arg35 + 0x38))()
                
                if (arg33 != 0)
                    Botan::deallocate_memory(arg33, arg34 - arg33, 1)
            void* x0_60 = arg_120
            
            if (x0_60 != 0)
                arg36 = x0_60
                operator delete(x0_60)
