...
final class ProxyNotificationInitializer {
    ...
    static void initialize(Context context0) {
        ...
        ProxyNotificationInitializer.setEnableProxyNotification(new EnhancedIntentService..ExternalSyntheticLambda0(), context0, ProxyNotificationInitializer.shouldEnableProxyNotification(context0));
    }

    static boolean isProxyNotificationEnabled(Context context0) {
        if(!ProxyNotificationInitializer.allowedToUse(context0)) {
            Log.e("FirebaseMessaging", "error retrieving notification delegate for package com.templegatesgames.DominionAndroid");
            return false;
        }
    ...
    static Task setEnableProxyNotification(Executor executor0, Context context0, boolean z) {
        ...
        executor0.execute(() -> try {
            if(!ProxyNotificationInitializer.allowedToUse(context0)) {
                Log.e("FirebaseMessaging", "error configuring notification delegate for package com.templegatesgames.DominionAndroid");
                return;
            }
