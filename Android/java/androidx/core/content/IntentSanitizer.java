...
public class IntentSanitizer {
    static class Api15Impl {
        static Intent getSelector(Intent intent0) {
            return intent0.getSelector();
        }

        static void setSelector(Intent intent0, Intent intent1) {
            intent0.setSelector(intent1);
        }
    }
        ...
        static void sanitizeClipData(Intent intent0, Intent intent1, Predicate predicate0, boolean z, Predicate predicate1, Consumer consumer0) {
            ...
            for(int v = 0; v < clipData0.getItemCount(); ++v) {
                ...
                if(Build.VERSION.SDK_INT >= 0x1F) {
                    ...
                    Api16Impl.checkOtherMembers(v, clipData$Item0, consumer0);
                }
                if(z) {
    ...
    public Intent sanitize(Intent intent0, Consumer consumer0) {
        ...
        Api16Impl.sanitizeClipData(intent0, intent1, this.mAllowedClipData, this.mAllowClipDataText, this.mAllowedClipDataUri, consumer0);
        ...
        if(this.mAllowSelector) {
            Api15Impl.setSelector(intent1, Api15Impl.getSelector(intent0));
        }
        else if(Api15Impl.getSelector(intent0) != null) {
            consumer0.accept("Selector is not allowed: " + Api15Impl.getSelector(intent0));
        }
